...
public class FlightSearch
{
    ...
    void demo()
    {
        FlightSearchCriteria searchCriteria = new FlightSearchCriteria("BCN", "MAD", timeProvider.getCurrentDateTime().plusDays(5), 2);
        List<FlightPrice> flightPrices = flightSearchService.searchFlightPrices(searchCriteria);

        System.out.println("Flights from BCN to MAD:");
        flightPrices.forEach(flightPrice ->  System.out.println(String.format("%s, %.2f %s", flightPrice.getFlightCode(), flightPrice.getPrice().getAmount(), flightPrice.getPrice().getCurrency().getSymbol())));
    }
}
